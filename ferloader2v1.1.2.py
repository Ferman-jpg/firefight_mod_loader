...
print(f"> Attempting to search for the mod folder in test 2 {trg_mod}...")
    ...
    if path.is_dir():
        ...
        if main_dir_Mod_origin != "":  #if the config file is not empty (a previous mod had just been loaded)
            ...
            if main_dir_Mod.is_dir():
                ...
                print("Mod folder exist in the main directory but no loading has ever happened")
    ...
    if path.is_dir():
        ...
        if cm_config_name_list != "":  # Map list is NOT empty (a mod is loaded in there)
            ...
            for map_name in map_to_unload:
                shutil.move(cm_main_dir / map_name, cm_mod_dir)
